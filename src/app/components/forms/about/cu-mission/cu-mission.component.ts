...
@Component({
    ...
    templateUrl: './cu-mission.component.html',
    styleUrl: './cu-mission.component.css'
})
export class CuMissionComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        private _request: MissionsService,
        @Inject(MAT_DIALOG_DATA) public data: any,
        @Inject(PLATFORM_ID) public _plateFormId: Object,
        @Inject(DOCUMENT) public _document: Document,
        private _dialogRef: MatDialogRef<CuMissionComponent>,
...
// __------__ 🍀🍀🍀🍀🍀__---   🍀 START EVENT 🍀   ---__ 🍀🍀🍀🍀🍀__------__//
    ...
    stepIs(step: number){
        if(step){
            if(this.localStorage){
                let data = this.localStorage.getItem('miss__fs__data');
                if(!data){
                    this.saveToLs();
                    setTimeout(()=>{
                        this.is_step = step;
        ...
        if(isPlatformBrowser(this._plateFormId)){
            ...
            .subscribe(_=>{
                // if(e){
                    ...
                    localStorage.setItem('miss__fs__data', JSON.stringify(data));
                // }
            })
        }
    }
    ...
    removeToLs(){
        if(isPlatformBrowser(this._plateFormId)){
            if(this.localStorage){
                this.localStorage.removeItem('miss__fs__data');
            }
        }
    }
    // close
