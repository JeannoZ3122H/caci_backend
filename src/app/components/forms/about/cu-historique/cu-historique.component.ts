...
@Component({
    ...
    templateUrl: './cu-historique.component.html',
    styleUrl: './cu-historique.component.css'
})
export class CuHistoriqueComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    constructor(
        ...
        private _request: HistoriqueService,
        @Inject(MAT_DIALOG_DATA) public data: any,
        @Inject(PLATFORM_ID) public _plateFormId: Object,
        @Inject(DOCUMENT) public _document: Document,
        private _dialogRef: MatDialogRef<CuHistoriqueComponent>,
...
// __------__ 🍀🍀🍀🍀🍀__---   🍀 START EVENT 🍀   ---__ 🍀🍀🍀🍀🍀__------__//
    ...
    stepIs(step: number){
        if(step){
            if(this.localStorage){
                let data = this.localStorage.getItem('histo__fs__data');
                if(!data){
                    this.saveToLs();
                    setTimeout(()=>{
                        this.is_step = step;
        ...
        if(isPlatformBrowser(this._plateFormId)){
            ...
            .subscribe(_=>{
                // if(e){
                    ...
                    localStorage.setItem('histo__fs__data', JSON.stringify(data));
                // }
            })
        }
    }
    ...
    getToLs(data: any){
        if(isPlatformBrowser(this._plateFormId)){
            if(this.localStorage){
                ...
                // this.readFileAsDataURL(data.illustration).subscribe({
                //     next: (dataURL) => {
    ...
    removeToLs(){
        if(isPlatformBrowser(this._plateFormId)){
            if(this.localStorage){
                this.localStorage.removeItem('histo__fs__data');
            }
        }
    }
    // close
