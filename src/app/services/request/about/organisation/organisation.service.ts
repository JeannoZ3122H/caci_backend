...
// __--__ 🍀🍀__-   🍀 START ENDPOINT BODY XHR 🍀   -__ 🍀🍀__--__//
    ...
    // 🍀🍀__-  Add New In List 🍀
        add(data: any) {
            let url = this.__apiUrl.apiUrl + "add_new_organisation";
            return this.__http.post(url, data, this.__apiUrl.getHeaders());
        }

    // 🍀🍀__-  Update In List 🍀
        update(data: any, slg: string) {
            let url = this.__apiUrl.apiUrl + `update_current_organisation/${slg}`;
            return this.__http.post(url, data, this.__apiUrl.getHeaders());
        }

    // 🍀🍀__-  Get All List 🍀
        get() {
            let url = this.__apiUrl.apiUrl + "get_list_organisations";
            return this.__http.get(url, this.getHeaders());
        }

    // 🍀🍀__-  Delete In List 🍀
        delete(slg: string) {
            let url = this.__apiUrl.apiUrl + `delete_current_organisation/${slg}`;
