...
@Component({
    ...
    templateUrl: './devenir-arbitre.component.html',
    styleUrl: './devenir-arbitre.component.css'
})
export class DevenirArbitreComponent implements OnInit, OnDestroy {
    private unscribe = new Subscription();
    public list_devenir: any[] = [];
    p: number = 1;
    constructor(
...
// __------__ 🍀🍀🍀🍀🍀__---   🍀 START REQUEST 🍀   ---__ 🍀🍀🍀🍀🍀__------__//
    ...
    getList() {
        this.unscribe.add(
            this.__request.getDevenir().subscribe(
                {
                    next: (resp: any) => {
                        if (resp) {
                            // console.log(resp)
                            this.list_devenir = resp;
                        }
                    },
                    error: (err: any) => {
                        if (err.status == 401) {
    ...
    delete(slg: any) {
        this.unscribe.add(
            this.__request.deleteDevenir(slg).subscribe(
                {
                    next: (resp: any) => {
                        if (resp.code == 100) {
        ...
        let dialog = this.__dialog.open(CuInfoPratiqueComponent,
            {
                ...
                data: { type: 'add', element: 'devenir', title: 'comment devenir arbitre médiation expert' }
            }
        )
        this.unscribe.add(
        ...
        let dialog = this.__dialog.open(CuInfoPratiqueComponent,
            {
                ...
                enterAnimationDuration: '300ms',
                exitAnimationDuration: '200ms',
                data: { type: 'edit', content: data, element: 'devenir', title: 'comment devenir arbitre médiation expert' }
            }
        )
        this.unscribe.add(
            dialog.afterClosed().subscribe(
        ...
        let dialog = this.__dialog.open(DocumentJoinsComponent,
            {
                ...
                data: {
                    ...
                    element: 'devenir',
                    title: 'comment devenir arbitre médiation expert'
                }
            }
        )
        this.unscribe.add(
